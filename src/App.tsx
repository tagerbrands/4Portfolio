      ...
      <div className="flex flex-col md:flex-row items-start md:items-start justify-between mb-8 gap-6 md:gap-4 w-full">
        <div className="w-full md:w-auto flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button 
              onClick={() => { 
                setEvlName(t("EvL2, studentversie", "Set2, student version")); 
            ...
            >
              {t("EvL2, studentversie", "Set2, student version")}
            </button>
            <button 
              onClick={() => { 
...
function Step2Proxies({ 
  ...
  const lo = learningOutcomes.find(l => l.id === loId);
  const loText = lo ? t(lo.text) : parts.map(p => t(p.text)).join(' ');

  const getPartTextColor = (part: LOPart, index: number) => {
    const classTextColor = part.colorClass?.split(' ').find(c => c.startsWith('text-'));
    if (classTextColor) {
      if (classTextColor === 'text-yellow-900') return 'text-amber-700';
  ...
  const renderFullLOWithHighlight = () => {
    if (!loText) return null;
    if (parts.length === 0) return loText;

    let elements: React.ReactNode[] = [loText];

    parts.forEach((part, partIdx) => {
      const isSelected = part.id === activePartId;
      const partText = t(part.text);
      if (!partText) return;

      const textColor = getPartTextColor(part, partIdx);

      elements = elements.flatMap((el, index) => {
        if (typeof el !== 'string') return el;

        const pieces = el.split(partText);
        if (pieces.length === 1) return el;

        const newElements: React.ReactNode[] = [];
        pieces.forEach((piece, i) => {
          if (piece) newElements.push(piece);
          if (i < pieces.length - 1) {
            newElements.push(
              <span
                key={`${part.id}-${index}-${i}`}
                ...
                className={`cursor-pointer transition-colors duration-150 select-none ${textColor} ${
                  isSelected
                    ? 'font-black underline decoration-current decoration-2 underline-offset-2'
                    : 'font-bold opacity-80 hover:opacity-100 hover:underline decoration-1 underline-offset-2'
                }`}
                title={isSelected ? `${t("Geselecteerd onderdeel")} ${partIdx + 1}` : `${t("Klik om te selecteren")}`}
              >
                {partText}
              </span>
            );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between mb-6 pb-4 border-b border-black/10 gap-4">
        <div>
          ...
          <div className="glass-panel overflow-hidden border border-white/60 shadow-md">
            ...
            {activePart ? (
              <div className="p-6 flex flex-col justify-between min-h-[420px]">
                <div>
                  <div className="mb-4 p-3 rounded-lg bg-black/[0.02] border border-black/10 text-sm leading-relaxed text-gray-800">
                    <span className="font-extrabold text-[var(--color-accent)] mr-2 select-none">
                      {loId}
                    </span>
                    {renderFullLOWithHighlight()}
                  </div>
        ...
        <div className="lg:col-span-1">
          <div className="glass-panel p-4 bg-white/40 sticky top-6 border border-white/60">
            ...
            <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1 scrollbar-hide">
              {parts.length === 0 ? (
                <div className="opacity-50 italic text-[11px] bg-black/5 p-3 rounded-lg text-center">
                  {t("Geen onderdelen gedefinieerd.")}
                </div>
              ) : (
                parts.map((p, pIndex) => {
                  const isSelected = p.id === activePartId;
                  const evList = p.evidence || [];
                  return (
                    <div 
                      key={p.id} 
                      className={`mb-3 last:mb-0 p-2.5 rounded-xl transition-all border ${
                        isSelected 
                          ? 'bg-white border-[var(--color-accent)] ring-2 ring-[var(--color-accent)]/20 shadow-md' 
                          : 'bg-black/[0.02] border-black/5 hover:bg-white/60 hover:border-black/10'
                      }`}
                    >
                      <button
                        type="button"
                        ...
                        className={`w-full flex items-center justify-between text-left p-1 rounded-lg transition-colors cursor-pointer group ${
                          isSelected ? 'text-[var(--color-accent)]' : 'text-gray-700 hover:text-black'
                        }`}
                        title={t("Klik om dit onderdeel te selecteren")}
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-extrabold shrink-0 shadow-xs transition-transform group-hover:scale-110 ${p.colorClass}`}>
                            {pIndex + 1}
                          </span>
                          <span className={`text-xs font-bold truncate ${isSelected ? 'font-extrabold text-[var(--color-accent)]' : ''}`}>
                            {t("Onderdeel")} {pIndex + 1}
                          </span>
                        </div>
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0 ${
                          evList.length > 0 
                            ? (isSelected ? 'bg-[var(--color-accent)]/10 text-[var(--color-accent)] font-bold' : 'bg-black/5 text-gray-600')
                            : 'bg-amber-50 text-amber-700 font-medium'
                        }`}>
                          {evList.length > 0 ? `${evList.length} ${evList.length === 1 ? t("bewijs") : t("bewijzen")}` : t("Geen bewijs")}
                        </span>
                      </button>
                      ...
                      {evList.length === 0 ? (
                        <button
                          ...
                          className="w-full text-left text-[11px] text-gray-400 italic py-1 px-2 rounded hover:text-[var(--color-accent)] cursor-pointer transition-colors"
                        >
                          + {t("Voeg bewijs toe...")}
                        </button>
                      ) : (
                          ...
                          {evList.map(ev => {
                            ...
                            return (
                              <div 
                                ...
                                className={`flex items-start justify-between p-2 rounded-lg border cursor-pointer hover:shadow-md transition-all ${typeInfo?.colorClass} ${editingEvidenceId === ev.id ? 'ring-2 ring-[var(--color-accent)] shadow-lg scale-[1.02]' : ''}`}
                              >
                                <div className="flex items-start gap-2 min-w-0 pointer-events-none">
                                  <div className="bg-white/50 p-1 rounded-md shadow-sm shrink-0 mt-0.5">
                                    <Icon className="w-3.5 h-3.5" />
                                  </div>
                                  <div className="min-w-0">
                                    <div className="font-bold text-[11px] leading-tight break-words">{t(ev.name)}</div>
                                    {ev.stakeholders && <div className="text-[10px] opacity-80 mt-0.5 break-words">👥 {ev.stakeholders.split(',').map(s => t(s.trim())).join(', ')}</div>}
                                  </div>
                                </div>
                                <button 
